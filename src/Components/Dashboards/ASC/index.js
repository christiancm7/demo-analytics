...
const ASCDashboard = () => {
  ...
  const chart = () => {
    ...
    let CPTLabels = [];
    let CPTData = [];
    let orderingPhysicianData = [];
    let orderingPhysicianLabel = [];
    ...
    axios
      ...
      .then((response) => {
        ...
        let billed = response.data[2].SeriesValue.split(",");
        let paid = response.data[4].SeriesValue.split(",");
        let denied = response.data[6].SeriesValue.split(",");
        let patientResp = response.data[8].SeriesValue.split(",");
        let underPayment = response.data[52].SeriesValue.split(",");
        let CPTCode = response.data[46].SeriesValue.split(",");
        let orderingPhysician = response.data[27].SeriesValue.split(",");
        ...
        for (let i = 0; i < 5; i++) {
          orderingPhysicianData[i] = orderingPhysician[i].split("|")[1];
          orderingPhysicianLabel[i] = orderingPhysician[i].split("|")[0];
        }
        ...
        setOrderingPhysicianData({
          labels: orderingPhysicianLabel,
          datasets: [
            {
              data: orderingPhysicianData,
              backgroundColor: ["#8dace7", "#71deb9", "#ef869e", "#cc65fe"],
              hoverBackgroundColor: [
                "#7097e1",
                "#4dd6a7",
  ...
  return (
    <Fragment>
      ...
      >
        <PageTitle
          heading="ASC Dashboard"
          subheading="Your ASC Dashboard"
          icon="pe-7s-graph opacity-6"
        />
        <TopRow data={kpiData} />
        <FullWidthBar
          data={chartData}
          name="HISTORICAL CLAIM TRENDS (12 MONTH MAX)"
        />
        {/* Bottom Row */}
        <BottomRow
          underPaymentData={underpaymentData}
          CPTCodeData={CPTCodeData}
          orderingPhysicianData={orderingPhysicianData}
        />
        {/* Bottomn Card End */}
      </CSSTransitionGroup>
    </Fragment>
