...
const InpatientDashboard = () => {
  ...
  const [DRGCodeData, setDRGCodeData] = useState({});
  ...
  const chart = () => {
    ...
    axios
      .get(
        "https://dev-emrreporting.axcension.com/api/dashboard/getfinancialchartitems/inpatient/imedx/all"
      )
      .then((response) => {
        ...
        let DRGCode = response.data[42].SeriesValue.split(",");
        ...
        for (let i = 0; i < 2; i++) {
          DRGData[i] = DRGCode[i].split("|")[1];
          DRGLabels[i] = DRGCode[i].split("|")[0];
        }
